...
class Reboot extends React.Component {
  triggerReboot = () => {
    ...
    this.props.notificationManager.current.openNotification(
      "Reboot has been started, please wait 5 minutes"
    );
    fetch(url, {
      method: "GET",
    })
      .then(() => {
        console.log(
          "reboot doesn't return anything, you should never be here!"
        );
      })
      .catch((error) =>
        this.props.notificationManager.current.openNotification(
          "Reboot is still in progress"
        )
      );
  };
